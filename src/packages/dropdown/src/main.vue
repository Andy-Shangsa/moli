...
export default {
  ...
  watch: {
    visible(val) {
      this.broadcast("MlDropdownMenu", "visible", val);
      this.$emit("visible-change", val);
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector(".ml-dropdown-selfdefine");
      if (selfDefine) {
        // 自定义
        if (val) {
    ...
    let triggerElm = !splitButton
      ...
      : h("ml-button-group", [
          ...
          h(
            ...
            {
              ref: "trigger",
              class: "ml-dropdown__caret-button",
              attrs: {
                type: type,
                size: dropdownSize
              }
            },
            [h("i", { class: "ml-dropdown__icon ml-icon-arrow-down" })]
          )
        ]);
    return h(
      "div",
      {
        class: "ml-dropdown",
        directives: [{ name: "clickoutside", value: hide }]
      },
