  ...
  >
    ...
    >
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="ml-input__icon ml-icon-close-fill"
    ...
    >
      ...
      <input
        ...
        type="text"
        class="ml-cascader__search-input"
    ...
    >
      <div
        ...
        ref="popper"
        :class="['ml-popper', 'ml-cascader__dropdown', popperClass]"
      >
        <ml-cascader-panel
          ref="panel"
          v-show="!filtering"
        ...
        <ml-scrollbar
          ...
          v-show="filtering"
          tag="ul"
          class="ml-cascader__suggestion-panel"
          view-class="ml-cascader__suggestion-list"
        ...
        >
          ...
          <slot
            ...
            name="empty"
          >
            <li class="ml-cascader__empty-text">{{ t('moli.cascader.noMatch') }}</li>
          </slot>
        </ml-scrollbar>
      </div>
    </transition>
...
import MlCascaderPanel from "../../cascader-panel";
...
export default {
  ...
  props: {
    ...
    placeholder: {
      type: String,
      default: () => t("moli.cascader.placeholder")
    },
    disabled: Boolean,
    clearable: Boolean,
    filterable: Boolean,
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          ...
          const firstMenu = popper.querySelector(".ml-cascader-menu");
          firstNode = firstMenu.querySelector(
            '.ml-cascader-node[tabindex="-1"]'
          );
        }
        if (firstNode) {
          firstNode.focus();
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector(".ml-input__inner");
      if (!inputInner) return;
      const tags = $el.querySelector(".ml-cascader__tags");
      let suggestionPanelEl = null;
      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector(
          ".ml-cascader__suggestion-list"
        );
