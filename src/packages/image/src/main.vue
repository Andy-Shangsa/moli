<template>
  <div class="ml-image">
    <slot
      ...
      name="placeholder"
    >
      <div class="ml-image__placeholder"></div>
    </slot>
    <slot
      v-else-if="error"
      name="error"
    >
      <div class="ml-image__error">{{ t('moli.image.error') }}</div>
    </slot>
    <img
      v-else
      class="ml-image__inner"
      v-bind="$attrs"
      v-on="$listeners"
      @click="clickHandler"
      :src="src"
      :style="imageStyle"
      :class="{ 'ml-image__inner--center': alignCenter, 'ml-image__preview': preview }"
    >
