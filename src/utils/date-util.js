...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`moli.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => t(`moli.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month => t(`moli.datepicker.months.${month}`)),
    monthNames: months.map((month, index) =>
      t(`moli.datepicker.month${index + 1}`)
    ),
    amPm: ["am", "pm"]
  };
};
