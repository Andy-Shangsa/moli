<template>
  <div
    class="ml-select"
    :class="[selectSize ? 'ml-select--' + selectSize : '']"
    @click.stop="toggleMenu"
    v-clickoutside="handleClose"
  >
    <div
      class="ml-select__tags"
      v-if="multiple"
      ref="tags"
    ...
    >
      <span v-if="collapseTags && selected.length">
        <ml-tag
          ...
          disable-transitions
        >
          <span class="ml-select__tags-text">{{ selected[0].currentLabel }}</span>
        </ml-tag>
        <ml-tag
          ...
          disable-transitions
        >
          <span class="ml-select__tags-text">+ {{ selected.length - 1 }}</span>
        </ml-tag>
      </span>
      <transition-group
      ...
      >
        <ml-tag
          ...
          disable-transitions
        >
          <span class="ml-select__tags-text">{{ item.currentLabel }}</span>
        </ml-tag>
      </transition-group>

      <input
        type="text"
        class="ml-select__input"
    ...
    >
      ...
      <template slot="suffix">
        <i
          v-show="!showClose"
          :class="['ml-select__caret', 'ml-input__icon', 'ml-icon-' + iconClass]"
        ></i>
        <i
          v-if="showClose"
          class="ml-select__caret ml-input__icon ml-icon-close-fill"
    ...
    >
      ...
      >
        <ml-scrollbar
          tag="ul"
          wrap-class="ml-select-dropdown__wrap"
          view-class="ml-select-dropdown__list"
          ref="scrollbar"
        ...
        <template v-if="emptyText && (!allowCreate || loading || (allowCreate && options.length === 0 ))">
          ...
          ></slot>
          <p
            class="ml-select-dropdown__empty"
            v-else
          >
            {{ emptyText }}
          </p>
...
import MlSelectMenu from "./select-dropdown.vue";
import MlOption from "./option.vue";
import MlTag from "../../tag";
...
export default {
    ...
    emptyText() {
      if (this.loading) {
        ...
        if (
          ...
          return this.noMatchText || this.t("moli.select.noMatch");
        }
        if (this.options.length === 0) {
          return this.noDataText || this.t("moli.select.noData");
        }
      }
      return null;
    },
  ...
  props: {
    ...
    placeholder: {
      type: String,
      default() {
        return t("moli.select.placeholder");
      }
    },
  ...
  watch: {
    ...
    visible(val) {
      if (!val) {
        this.broadcast("MlSelectDropdown", "destroyPopper");
        if (this.$refs.input) {
        ...
        if (this.filterable) {
          ...
          if (this.multiple) {
            ...
            if (!this.remote) {
              this.broadcast("MlOption", "queryChange", "");
              this.broadcast("MlOptionGroup", "queryChange");
            }
  ...
  methods: {
    ...
    handleQueryChange(val) {
      ...
      this.previousQuery = val;
      this.$nextTick(() => {
        if (this.visible) this.broadcast("MlSelectDropdown", "updatePopper");
      });
      this.hoverIndex = -1;
      ...
      if (this.remote && typeof this.remoteMethod === "function") {
        ...
        this.broadcast("MlOption", "queryChange", val);
        this.broadcast("MlOptionGroup", "queryChange");
      }
      if (
        this.defaultFirstOption &&
