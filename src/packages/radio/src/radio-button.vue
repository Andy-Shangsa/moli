<template>
  <label
    class="ml-radio-button"
    :class="[
      size ? 'ml-radio-button--' + size : '',
  ...
  >
    <input
      class="ml-radio-button__orig-radio"
      :value="label"
      type="radio"
      v-model="value"
      :name="name"
...
export default {
  ...
  methods: {
    handleChange() {
      this.$nextTick(() => {
        this.dispatch("MlRadioGroup", "handleChange", this.value);
      });
    }
  }
};
