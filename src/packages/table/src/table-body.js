...
export default {
  ...
  watch: {
    ...
    "store.states.hoverRow"(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll(".ml-table__row");
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push("ml-table__row--striped");
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, "ml-tooltip") && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(h, row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push("ml-table__row--level-" + treeRowData.level);
